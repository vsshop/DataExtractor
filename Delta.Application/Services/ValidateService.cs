...
public class ValidateService(IHostEnvironment environment, XLSXReaderService reader)
{
    public async Task<List<ValidateOperation>> Validate()
    {
        var files = Directory.GetFiles(environment.ValidationRules);
        ...
        return result.Value!.Tables[0].Select<ValidateOperation>();
    }
}
